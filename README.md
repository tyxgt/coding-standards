...
├── backend-standards/     # 🚧 后端规范（计划中）
└── database-standards/    # 🚧 数据库规范（计划中）
```

在每个新目录下创建规范文件，然后为每个工具创建对应的适配器模板放在 `adapters/<tool>/` 下，最后更新 `install.sh`。

> 🚧 **计划中**：`backend-standards/` 和 `database-standards/` 目录规划中，目前仅前端规范可用。当你需要添加新的规范类别时，按上述模式创建目录和适配器即可。
...
# 2. 从规范源重新生成适配器
bash scripts/generate-adapters.sh --generate

# 3. 验证适配器与规范源一致
bash scripts/generate-adapters.sh --check
...
适配器中的 `010-frontend-core.mdc`、`codebuddy/frontend-standards.md` 等为从规范源自动生成的内容。修改规范源后，运行 `--generate` 即可同步：
...
# 生成并验证一步完成
bash scripts/generate-adapters.sh --generate --check
```

> **注意**：`generate-adapters.sh` 支持双模式：
> - `--check`（默认）：验证适配器是否与规范源一致，检测规则缺失和时间漂移
> - `--generate`：从 `frontend-standards/` 自动提取规则，生成适配器内联内容
