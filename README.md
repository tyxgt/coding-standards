...
> 一套规范源，适配多款 AI 编码工具。
> 当前覆盖：**Claude Code · Cursor · Trae · CodeBuddy · Amazon Q Developer**
...
```bash
# 将 ai-rules 放到你的项目目录下，然后运行：
bash ai-rules/scripts/install.sh --target-dir /path/to/your/project

# 或只安装特定工具：
bash ai-rules/scripts/install.sh --target-dir . --tools cursor,codebuddy

# 使用符号链接模式（规范更新后自动同步）：
bash ai-rules/scripts/install.sh --target-dir . --mode symlink
```
...
│   ├── component-patterns.md        # 组件编写规范
│   ├── import-organization.md       # 导入规范
│   ├── state-management.md          # 状态管理
│   ├── api-requests.md              # API 请求
│   ├── styling.md                   # 样式规范
│   ├── typescript.md                # TypeScript 规范
│   ├── comments.md                  # 注释规范
│   ├── lint-rules.md                # 代码质量规则
│   └── new-project-defaults.md      # 新项目默认规范
│
├── adapters/                        # 工具适配器模板
...
│   └── amazon-q/frontend-standards.md  # → .amazonq/rules/
│
└── scripts/
...
- **Cursor / CodeBuddy / Amazon Q**：规则内容直接内联在适配文件中，但经过高度浓缩（每个适配器 100-200 行），只包含最常用规则
...
| 注意事项 | 安装脚本会替换 `{{AI_RULES_PATH}}` 占位符 |
...
| 项目 | 说明 |
|------|------|
| 安装位置 | `.amazonq/rules/frontend-standards.md` |
| 加载方式 | 工作区启动时自动加载 |
| 规则读取 | 完全内联，约 80 行（最浓缩版本） |
| 注意事项 | 安装后**立即生效**，无需重启 |
...
适配器中的 `010-frontend-core.mdc`、`codebuddy/frontend-standards.md`、`amazon-q/frontend-standards.md` 为手写浓缩版本。修改规范源后，需要手动同步这些文件中的关键规则。

---

## FAQ

**Q: 为什么 Cursor/CodeBuddy/Amazon Q 的适配器是内联的，而 Claude Code/Trae 是用 Read 加载的？**

A: Cursor 目前不支持 `@file` 引用（功能已确认损坏），且没有等价的 Read 机制。Amazon Q 加载所有规则但没有文件引用能力。Claude Code 和 Trae 的 Skill 系统支持在 SKILL.md 中通过 Read 指令按需加载规范文件，因此可以采用渐进加载策略。

**Q: 安装脚本报 "无法计算相对路径"？**

A: macOS 缺少 `realpath --relative-to` 命令。可以手动指定路径：
```bash
bash ai-rules/scripts/install.sh --target-dir . --ai-rules-path ai-rules
```
...
```bash
bash ai-rules/scripts/install.sh --tools cursor,codebuddy,amazon-q
```
...
rm -f .amazonq/rules/frontend-standards.md
```
